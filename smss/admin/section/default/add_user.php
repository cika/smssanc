...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>เพิ่ม แก้ไข ผู้ใช้ (User)</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>เพิ่มผู้ใช้ (User)</Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขผู้ใช้ (User)</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
echo "<Table width='50%' Border= '0' Bgcolor='#Fcf9d8'>";
...
$url_link="file=add_user&name_search=$_REQUEST[name_search]";  // 2_กำหนดลิงค์

if($_REQUEST['name_search']!=""){
$sql = "select system_user.id, system_user.username, system_user.userpass, system_user.smss_admin, system_user.status, person_main.name, person_main.surname, person_main.person_id from system_user left join person_main on system_user.person_id=person_main.person_id where person_main.name like '%$_REQUEST[name_search]%' or system_user.username like '%$_REQUEST[name_search]%' ";
}
else{
$sql = "select * from system_user"; // 3_กำหนด sql
}
...
if($_REQUEST['name_search']!=""){
$sql = "select system_user.id, system_user.username, system_user.userpass, system_user.smss_admin, system_user.status, person_main.name, person_main.surname, person_main.person_id from system_user left join person_main on system_user.person_id=person_main.person_id where person_main.name like '%$_REQUEST[name_search]%' or system_user.username like '%$_REQUEST[name_search]%' order by person_main.name limit $start,$pagelen";
}
else{
$sql = "select system_user.id, system_user.username, system_user.userpass, system_user.smss_admin, system_user.status, person_main.name, person_main.surname, person_main.person_id from system_user left join person_main on system_user.person_id=person_main.person_id order by system_user.username limit $start,$pagelen";
}

$dbquery = mysql_query($sql);
echo  "<table width=75% border=0 align=center>";
echo "<Tr bgcolor='#FFCCCC'><Td  align='center' width='50'>ที่</Td><Td align='center'>Username</Td><Td  align='center'>ชื่อ-สกุล</Td><Td align='center' width='170'>สถานะผู้ดูแลระบบ(Admin)</Td><Td align='center' width='100'>สถานะใช้งาน</Td><Td align='center' width='50'>ลบ</Td><Td align='center' width='50'>แก้ไข</Td></Tr>";
...
While ($result = mysql_fetch_array($dbquery))
	{
		...
		echo "<Tr bgcolor=$color><Td align='center'>$N</Td><Td align='left'>$username</Td>
