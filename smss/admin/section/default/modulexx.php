...
$(function(){
	$("select#module").change(function(){
		...
		$("div#displayAJAX").html(datalist2); // นำค่า datalist2 มาแสดงใน displayAJAX
...
//ส่วนหัว
echo "<br />";
if(!(($index==1) or ($index==1.1) or ($index==2) or ($index==5))){
...
echo "</form>";
echo "<br>";
echo "<div align='center'>*กรณีเลือกชื่อระบบงานย่อย(อังกฤษ) ไม่ได้ <a href=?file=module&index=1.1>คลิก</a></div>";
}
...
$dbquery_module = mysql_db_query($dbname, $sql);
$module_num=mysql_num_rows($dbquery_module);
	...
	$sql = "insert into system_module (module, module_desc, workgroup, module_active) values ('$_POST[module]', '$_POST[module_desc]','$_POST[workgroup]','1')";
	$dbquery = mysql_db_query($dbname, $sql);
	}
}
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไข Admin ระบบงานย่อย</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
echo "<Table width='50%' Border= '0' Bgcolor='#Fcf9d8'>";
...
if ($index==6){
$rec_date = date("Y-m-d");
$sql = "update system_module set  module_desc='$_POST[module_desc]', workgroup='$_POST[workgroup]', module_active='$_POST[module_active]' where id='$_POST[id]'";
$dbquery = mysql_db_query($dbname, $sql);
}
if ($index==6.5){
...
While ($result = mysql_fetch_array($dbquery)){
	...
	$dbquery_order = mysql_db_query($dbname, $sql_order);
$order_number++;
	}
}
...
$sql = "select system_module.id,system_module.module,system_module.module_desc,system_module.module_active,system_workgroup.workgroup_desc from system_module left join system_workgroup on system_module.workgroup=system_workgroup.workgroup order by system_module.module_order limit $start,$pagelen";
$dbquery = mysql_db_query($dbname, $sql);
$num_effect = mysql_num_rows($dbquery );  // จำนวนข้อมูลในหน้านี้
echo  "<table width=85% border=0 align=center>";
echo "<Tr><Td colspan='7' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มระบบงานย่อย (Module)' onclick='location.href=\"?file=module&index=1\"'</Td></Tr>";

echo "<Tr bgcolor='#FFCCCC'><Td  align='center' width='50'>ที่</Td><Td  align='center' width='170'>ชื่อระบบงานย่อย(อังกฤษ)</Td><Td  align='center'>ชื่อระบบงานย่อย(ไทย)</Td><Td  align='center'>กลุ่ม(งาน)</Td><Td align='center'  width='50'>ลำดับ</Td><Td align='center'  width='90'>สถานะใช้งาน</Td><Td align='center'  width='50'>ลบ</Td><Td align='center' width='50'>แก้ไข</Td></Tr>";
...
While ($result = mysql_fetch_array($dbquery))
	{
		...
		echo "<Tr bgcolor=$color><Td align='center'>$N</Td><Td  align='left'>$module</Td><Td align='left'>$module_desc</Td><Td align='left'>$workgroup_desc</Td>";
