...
#		$a_val=array("0" => "C", "1" => "W", "2" => "S", "3" => "L");
		...
		$nv="";
			for($q=0;$q<count($a_val);$q++)
			{
						...
						$nv[$q]=	mysql_num_rows(mysql_query($l));
			}
			$sv="";
			$all=0;
...
echo "<tr><td align='center'><font color='#990000' size='4'>รายละเอียดการมาเรียนของนักเรียน $edu_level[$class_now]$room_now   ปีการศึกษา $txtyear_active</font><br> <font color='#006666' size='4'>ประจำวันที่ 
...
<TR  bgcolor='#FFFFFF'>
	<TD colspan=5><a href="javascript:history.back();"> |<< ย้อนกลับ</a></TD>
</TR>
<TR  bgcolor='#FFCCCC'>
...
$dbquery = mysql_query( $sql);#echo $sql;echo $page;
echo  "<table width=65% border=0 align=center>";
echo "<Tr bgcolor='#FFCCCC'>
