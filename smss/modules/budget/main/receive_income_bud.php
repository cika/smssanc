...
$budget_status['1']="เงินสด";
$budget_status['2']="เช็ค/เงินฝากธนาคาร";
$budget_status['3']="เงินฝากส่วนราชการผู้เบิก";
$budget_status['4']="นำเงินสดฝากธนาคาร";
$budget_status['5']="นำเงินสดฝากส่วนราชการผู้เบิก";
$budget_status['6']="ถอนเงินฝากธนาคารเป็นเงินสด";
$budget_status['7']="ถอนเงินฝากธนาคารไปฝากส่วนราชการผู้เบิก";
$budget_status['8']="รับคืนเงินฝากส่วนราชการผู้เบิกมาเป็นเงินสด";
$budget_status['9']="รับคืนเงินฝากส่วนราชการมาเป็นเงินฝากธนาคาร";
...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนรับเงินรายได้แผ่นดิน ปีงบประมาณ$year_active_result[budget_year]</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>เพิ่มข้อมูลการรับเงินรายได้แผ่นดิน ปีงบประมาณ$year_active_result[budget_year]</Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=budget&task=main/receive_income_bud&index=3&id=$_GET[id]&page=$_REQUEST[page]&type_id_index=$_REQUEST[type_id_index]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=budget&task=main/receive_income_bud&page=$_REQUEST[page]&type_id_index=$_REQUEST[type_id_index]\"'";
echo "</td></tr></table>";
}
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขทะเบียนรับเงินรายได้แผ่นดิน ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
...
if ($index==7){
echo "<Center>";
echo "<Font color='#006666' Size=3><B>รายละเอียดรายการรับเงินรายได้แผ่นดิน ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br>";
$sql = "select * from budget_main where id='$_GET[id]'";
$dbquery = mysql_query($sql);
...
echo "<Tr ><Td colspan='7' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=budget&task=main/receive_income_bud&page=$_GET[page]\"'></Td></Tr>";
...
$sql = "select id,receive_amount from budget_main where budget_year='$year_active_result[budget_year]' and type_id between '300' and '399' and status between '1' and '2' order by rec_date, id";
}

$receive_total=0; //รวมรับ
$dbquery = mysql_query($sql);
...
//ส่วนของการแยกหน้า
$pagelen=20;  // 1_กำหนดหน้าฺ
$url_link="option=budget&task=main/receive_income_bud";  // 2_กำหนดลิงค์ฺ
...
//จบแยกหน้า
		...
		echo "<Tr><Td align='left'><INPUT TYPE='button' name='smb' value='เพิ่มรายการรับ' onclick='location.href=\"?option=budget&task=main/receive_income_bud&index=1\"'></Td>";
		echo "<td>";
		//เลืิอกประเภท
		echo "<form id='frm1' name='frm1'>";
...
if($_REQUEST['type_id_index']!=""){
$sql = "select budget_main.id, budget_main.doc, budget_type.type_name, budget_main.item, budget_main.receive_amount, budget_main.status, budget_main.rec_date,budget_main.officer from  budget_main left join  budget_type on budget_main.type_id=budget_type.type_id  where budget_main.budget_year='$year_active_result[budget_year]' and budget_main.type_id between '300' and '399' and budget_main.status between '1' and '2' and budget_main.type_id='$_REQUEST[type_id_index]' order by budget_main.rec_date, budget_main.id limit $start,$pagelen";
}
else{
$sql = "select budget_main.id, budget_main.doc, budget_type.type_name, budget_main.item, budget_main.receive_amount, budget_main.status, budget_main.rec_date,budget_main.officer from  budget_main left join  budget_type on budget_main.type_id=budget_type.type_id  where budget_main.budget_year='$year_active_result[budget_year]' and budget_main.type_id between '300' and '399' and budget_main.status between '1' and '2' order by budget_main.rec_date, budget_main.id limit $start,$pagelen";
}
...
echo  "<table width='95%' border='0' align='center'>";
echo "<Tr bgcolor=#FFCCCC align='center'><Td width='30'>ที่</Td><Td width='70'>วดป</Td><Td width='70'>ที่เอกสาร</Td><Td>ประเภท</Td><Td>รายการ</Td><Td>ลักษณะรายการ</Td><Td width='90'>จำนวนเงิน</Td><Td>รายละเอียด</Td><Td width='40'>ลบ</Td><Td width='40'>แก้ไข</Td><td width='60'>รวม</td></Tr>";
$N=(($page-1)*$pagelen)+1;
$M=1;
While ($result = mysql_fetch_array($dbquery))
	{
...
$t_year=($rec_year+543)-2500;		
		...
		echo "<Tr bgcolor=$color><Td align='center'>$N</Td> <Td>$rec_day $t_month[$rec_month] $t_year</Td><Td align=left>$doc</Td><Td>$type_name</Td><Td align=left>$item";
		echo "</Td><Td>$budget_status[$status]</Td><Td align='right'>$receive_amount</Td>";
		echo "<Td width=90><div align=center><a href=?option=budget&task=main/receive_income_bud&id=$id&index=7&page=$page><img src=./images/browse.png border='0' alt='รายละเอียด'></a></td>";
		if($officer==$result['officer']){
		echo "<Td align=center><a href=?option=budget&task=main/receive_income_bud&index=2&id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]><img src=images/drop.png border='0' alt='ลบ'></a></Td>
		<Td align=center><a href=?option=budget&task=main/receive_income_bud&index=5&id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td>";
		}
		else{
		echo "<td>&nbsp;</td><td>&nbsp;</td>";
		}
		...
		if($_GET['cal_id']==$id){
		echo "<Td align=center><a href=?option=budget&task=main/receive_income_bud&page=$page&type_id_index=$_REQUEST[type_id_index]>$tungnee</a></Td>";
		echo "</Tr>";
		break;  //ออกจากloop
		}
		else {
		echo "<Td align=center><a href=?option=budget&task=main/receive_income_bud&cal_id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]>ถึงนี้</a></Td>";
		echo "</Tr>";
		}		
...
function goto_url(val){
	if(val==0){
		callfrm("?option=budget&task=main/receive_income_bud");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.doc.value == ""){
			...
			callfrm("?option=budget&task=main/receive_income_bud&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.update_date.value == ""){
			...
			callfrm("?option=budget&task=main/receive_income_bud&index=6");   //page ประมวลผล
		}
	}
}
