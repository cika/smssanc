...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=budget&task=main/budget_year&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=budget&task=main/budget_year&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}

//ส่วนลบข้อมูล
if($index==3){
$sql = "delete from budget_year where id=$_GET[id]";
...
if($index==4){
	...
	$sql = "update budget_year set  year_active='0' ";
	$dbquery = mysql_query($sql);
	}
$rec_date = date("Y-m-d");
$sql = "insert into budget_year (budget_year, year_active) values ('$_POST[budget_year]', '$_POST[year_active]')";
$dbquery = mysql_query($sql);
}
...
$sql = "select * from budget_year where id='$_GET[id]'";
$dbquery = mysql_query($sql);
...
$sql = "update budget_year set  budget_year='$_POST[budget_year]', year_active='$_POST[year_active]' where id='$_POST[id]'";
$dbquery = mysql_query($sql);
}
...
$sql = "update budget_year set  year_active='$year_active' where id='$_GET[id]'";
$dbquery = mysql_query($sql);
}
...
$url_link="option=budget&task=main/budget_year";  // 2_กำหนดลิงค์ฺ
$sql = "select * from budget_year"; // 3_กำหนด sql
...
$sql = "select * from  budget_year order by budget_year desc limit $start,$pagelen";
$dbquery = mysql_query($sql);
echo  "<table width=50% border=0 align=center>";
echo "<Tr><Td colspan='5' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มปีงบประมาณ' onclick='location.href=\"?option=budget&task=main/budget_year&index=1\"'</Td></Tr>";
...
While ($result = mysql_fetch_array($dbquery))
	{
		...
		echo "<Tr bgcolor=$color><Td align='center' width='50'>$N</Td><Td  align='center'>$budget_year</Td><Td align='center'><a href=?option=budget&task=main/budget_year&index=7&id=$id&year_active=$year_active&page=$page>$active_pic</a></Td>
		<Td align='center'><a href=?option=budget&task=main/budget_year&index=2&id=$id&page=$page><img src=images/drop.png border='0' alt='ลบ'></a></a></Td>
		<Td align='center'><a href=?option=budget&task=main/budget_year&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>
	</Tr>";
$M++;
...
function goto_url(val){
	if(val==0){
		callfrm("?option=budget&task=main/budget_year");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.budget_year.value == ""){
			...
			callfrm("?option=budget&task=main/budget_year&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.budget_year.value == ""){
			...
			callfrm("?option=budget&task=main/budget_year&index=6");   //page ประมวลผล
		}
	}
}
</script>
