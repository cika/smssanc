...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนการเปลี่ยนแปลงสถานะเงินรายได้แผ่นดิน ปีงบประมาณ$year_active_result[budget_year]</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>เปลี่ยนแปลงสถานะเงินรายได้แผ่นดิน ปีงบประมาณ$year_active_result[budget_year]</Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=budget&task=main/change_income_bud&index=3&id=$_GET[id]&page=$_REQUEST[page]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=budget&task=main/change_income_bud&page=$_REQUEST[page]\"'";
echo "</td></tr></table>";
}
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขการเปลี่ยนแปลงสถานะเงินรายได้แผ่นดิน ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
...
if ($index==7){
echo "<Center>";
echo "<Font color='#006666' Size=3><B>รายละเอียดการเปลี่ยนแปลงสถานะเงินรายได้แผ่นดิน ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
echo "<Table   width=60% Border=0 Bgcolor=#Fcf9d8>";
echo "<Tr ><Td colspan='11' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=budget&task=main/change_income_bud&page=$_GET[page]&type_id_index=$_REQUEST[type_id_index]\"'></Td></Tr>";
$sql = "select * from budget_main where id='$_GET[id]'";
$dbquery = mysql_query($sql);
$ref_result = mysql_fetch_array($dbquery);
list($rec_year,$rec_month,$rec_day) = explode("-",$ref_result['rec_date']);	
...
$url_link="option=budget&task=main/change_income_bud";  // 2_กำหนดลิงค์ฺ

$sql = "select  id  from budget_main where budget_year='$year_active_result[budget_year]' and type_id between '300' and '399' and status between '5' and '10'";
$dbquery = mysql_query($sql);
...
$t_year=($rec_year+543)-2500;		
		...
		echo "<Tr bgcolor=$color><Td align='center'>$N</Td> <Td>$rec_day $t_month[$rec_month] $t_year</Td><Td align=left>$doc</Td><Td>$type_name</Td><Td align=left>$item</Td><Td>$budget_status[$status]</Td><Td align='right'>$change_amount</Td><Td align='center'><a href=?option=budget&task=main/change_income_bud&index=7&id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]><img src=./images/browse.png border='0' alt='รายละเอียด'></a></Td>";
		if($officer==$result['officer']){		
		echo "<Td><div align='center'><a href=?option=budget&task=main/change_income_bud&index=2&id=$id&page=$page><img src=images/drop.png border='0' alt='ลบ'></a></div></Td>
		<Td align='center'><a href=?option=budget&task=main/change_income_bud&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td>
	</Tr>";
		}
		else{
		echo "<td>&nbsp;</td><td>&nbsp;</td></Tr>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=budget&task=main/change_income_bud");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.doc.value == ""){
			...
			callfrm("?option=budget&task=main/change_income_bud&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.update_date.value == ""){
			...
			callfrm("?option=budget&task=main/change_income_bud&index=6");   //page ประมวลผล
		}
	}
}
</script>
