...
$sql = "select  * from budget_main where budget_year='$year_active_result[budget_year]' and type_id<'200' order by rec_date, id";
}
$dbquery = mysql_query($sql);
...
echo "<font color='#006666' size='3'><strong>รายงานเงินนอกงบประมาณ</strong></font><br />";
...
$url_link="option=budget&task=main/ex_bud_book";  // 2_กำหนดลิงค์ฺ
...
$sql= "select * from  budget_main where  budget_year='$year_active_result[budget_year]' and type_id<'200' order by rec_date, id  limit $start,$pagelen";
}
$dbquery = mysql_query($sql);
...
$office_ar_display=number_format($office_ar[$id_num],2);	
		...
		echo "<Td align=center><a href=?option=budget&task=main/ex_bud_book&page=$page&type_id_index=$_REQUEST[type_id_index]&year_index=$year_active_result[budget_year]>$tungnee</a></Td>	</Tr>";
		break; //ออกจากloop
		}
		else {
		echo "<Td align=center><a href=?option=budget&task=main/ex_bud_book&cal_id=$id_num&page=$page&type_id_index=$_REQUEST[type_id_index]&year_index=$year_active_result[budget_year]>ถึงนี้</a></Td>	</Tr>";
		}
...
function goto_url(val){
	if(val==0){
		callfrm("?option=budget&task=main/ex_bud_book");   // page ย้อนกลับ 
	}else if(val==1){
			callfrm("?option=budget&task=main/ex_bud_book");   //page ประมวลผล
		}
}
