...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนการจ่ายเงินนอกงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>เพิ่มข้อมูลการจ่ายเงินนอกงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=budget&task=main/pay_ex_bud&index=3&id=$_GET[id]&page=$_REQUEST[page]&type_id_index=$_REQUEST[type_id_index]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=budget&task=main/pay_ex_bud&page=$_REQUEST[page]&type_id_index=$_REQUEST[type_id_index]\"'";
echo "</td></tr></table>";
}
...
if($index==4){
$rec_date = date("Y-m-d");
$sql = "insert into budget_main (budget_year, doc, refer_wd_id, type_id, item, pay_group, pay_amount, payed_person, status,  rec_date, officer) values ('$year_active_result[budget_year]', '$_POST[doc]','$_POST[refer_wd_id]', '$_POST[type_id]', '$_POST[item]', '$_POST[pay_group]','$_POST[pay_amount]', '$_POST[payed_person]', '$_POST[status]', '$rec_date','$officer')";
$dbquery = mysql_query($sql);
}
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขทะเบียนการจ่ายเงินนอกงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
...
pay_group='$_POST[pay_group]', status='$_POST[status]', officer='$officer' , rec_date='$rec_date' where id='$_POST[id]'";
$dbquery = mysql_query($sql);
}
...
echo "<form id='frm1' name='frm1'>";
echo "<Center>";
echo "<Font color='#006666' Size=3><B>รายละเอียดการจ่ายเงินนอกงบประมาณ ปีงบประมาณ$year_active_result[budget_year]</B></Font>";
echo "</Cener>";
echo "<Br><Br>";

echo "<Table   width=70% Border=0 Bgcolor=#Fcf9d8>";
echo "<Tr ><Td colspan='11' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=budget&task=main/pay_ex_bud&page=$_GET[page]&type_id_index=$_REQUEST[type_id_index]\"'></Td></Tr>";
...
$sql = "select id, pay_amount from  budget_main where budget_year='$year_active_result[budget_year]' and status between '3' and '4' and  type_id<'200' order by rec_date, id";
}
$dbquery = mysql_query($sql);
...
$url_link="option=budget&task=main/pay_ex_bud";
...
//จบแยกหน้า
		...
		echo "<Tr><Td align='left'><INPUT TYPE='button' name='smb' value='เพิ่มรายการจ่าย' onclick='location.href=\"?option=budget&task=main/pay_ex_bud&index=1\"'></Td>";
		echo "<td>";
...
$sql = "select budget_main.id, budget_main.doc,budget_main.refer_wd_id, budget_type.type_name, budget_main.item, budget_main.pay_amount, budget_main.pay_group, budget_main.status, budget_main.officer, budget_main.rec_date from  budget_main left join  budget_type on budget_main.type_id=budget_type.type_id  where budget_main.budget_year='$year_active_result[budget_year]' and budget_main.status between '3' and '4' and budget_main.type_id<'200' order by budget_main.rec_date, budget_main.id limit $start,$pagelen";
}
...
echo  "<table width=95% border=0 align=center>";
echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='30'>ที่</Td><Td width='70'>วดป</Td><Td width='70'>ที่เอกสาร</Td><Td>เลขที่อ้างอิง</Td><Td>รายการ</Td><Td>ลักษณะรายการ</Td><Td width='90'>จำนวนเงิน</Td><Td width='40'>รายละเอียด</Td><Td width='40'>ลบ</Td><Td width='40'>แก้ไข</Td><Td width='40'>รวม</Td></Tr>";
...
if($refer_wd_id>0){
echo "<a href=?option=budget&task=main/pay_ex_bud2&index=7&id=$refer_wd_id&page=$page&type_id_index=$_REQUEST[type_id_index]>$refer_wd_id</a>";
}
echo "</Td><Td align=left>$item</Td><Td>$budget_status[$status]</Td><Td align='right'>$pay_amount</Td><Td align='center'><a href=?option=budget&task=main/pay_ex_bud&index=7&id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]><img src=./images/browse.png border='0' alt='รายละเอียด'></a></Td>";
		if($officer==$result['officer']){
echo "<Td align='center'><a href=?option=budget&task=main/pay_ex_bud&index=2&id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]><img src=images/drop.png border='0' alt='ลบ'></a></Td>
		<Td align='center'><a href=?option=budget&task=main/pay_ex_bud&index=5&id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]><img src=images/edit.png border='0' alt='แก้ไข'></a></div></Td>";
		}
		else{
		echo "<td>&nbsp;</td><td>&nbsp;</td>";
		}
		...
		if($_GET['cal_id']==$id){
		echo "<Td align=center><a href=?option=budget&task=main/pay_ex_bud&page=$page&type_id_index=$_REQUEST[type_id_index]>$tungnee</a></Td>	</Tr>";
		break; //ออกจากloop
		}
		else {
		echo "<Td align=center><a href=?option=budget&task=main/pay_ex_bud&cal_id=$id&page=$page&type_id_index=$_REQUEST[type_id_index]>ถึงนี้</a></Td>	</Tr>";
		}
	//ออกจากloop
	if(isset($_GET['id'])){
	if($_GET['id']==$id){
...
function goto_url(val){
	if(val==0){
		callfrm("?option=budget&task=main/pay_ex_bud");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.doc.value == ""){
			...
			callfrm("?option=budget&task=main/pay_ex_bud&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.update_date.value == ""){
			...
			callfrm("?option=budget&task=main/pay_ex_bud&index=6");   //page ประมวลผล
		}
	}
}
