  ...
  <tr>
	...
	<table width="1000" border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td colspan="2" align="center"><strong>รายงาน แบบ BMI</strong><br>
        สุขภาพนักเรียนทั้งโรงเรียน ปีการศึกษา 
          <?php echo $year_bs;?></td>
        </tr>
         <form name="form1" action="?option=health&&task=report_all" method="post" enctype="multipart/form-data">
      <tr>
      ...
      <tr>
        <td width="438" align="right" valign="top">ภาคเรียนที่ 1 <?php if($ck==1 && $tm==1){ echo"<img src='modules/health/iconH/yes.png' width='15' height='15' border='0'>";}?>[<a href="?option=health&task=report_all&ck=1&tm=1&year_in=<?php echo $year_bs;?>">ครั้งที่ 1</a>]  <?php if($ck==2 && $tm==1){ echo"<img src='modules/health/iconH/yes.png' width='15' height='15' border='0'>";}?>[<a href="?option=health&task=report_all&ck=2&tm=1&year_in=<?php echo $year_bs;?>">ครั้งที่ 2</a>]&nbsp;&nbsp;</td>
        <td width="444" align="left" valign="top">&nbsp;&nbsp;ภาคเรียนที่ 2 <?php if($ck==1 && $tm==2){ echo"<img src='modules/health/iconH/yes.png' width='15' height='15' border='0'>";}?>[<a href="?option=health&task=report_all&ck=1&tm=2&year_in=<?php echo $year_bs;?>">ครั้งที่ 1</a>]  <?php if($ck==2 && $tm==2){ echo"<img src='modules/health/iconH/yes.png' width='15' height='15' border='0'>";}?>[<a href="?option=health&task=report_all&ck=2&tm=2&year_in=<?php echo $year_bs;?>">ครั้งที่ 2</a>]</td>
      </tr>
    </table></td>
  </tr>
  <tr>
	...
	<table width="1000" border="1" align="center" cellpadding="0" cellspacing="0">
      ...
      <tr>
        <td width="52" align="center" valign="middle" bgcolor="#76CEEB">ต่ำ</td>
        <td width="52" align="center" valign="middle" bgcolor="#76CEEB">ปกติ</td>
        <td width="51" align="center" valign="middle" bgcolor="#76CEEB">เกิน</td>
      ...
      <?php 
        ...
        $sqlslw="SELECT* FROM health_checking WHERE room='$recordSM[room]'&&class_now='$class_code'&&year_std='$year_bs'&&number_check='$ck'&&term_std='$tm'";
				...
				$checkslw=mysql_num_rows($resultslw);
						/*			if($checkslw==0 || $checkslw=="" || $checkslw==1)
								{   */
						$sum_low=0;
						$sum_nor=0;
						$sum_over=0;
	...
	{	
				...
				$tall=$recordslw['tall']; /*ส่วนสูง*/
					if($weight!=""||$tall!=""){
						...
						$total=$weight/$sum_tall;	/*หาค่า BMI*/
							$total_con =	number_format($total,2);
							  if($total_con < 18.50){
								  $action_show="ผอม";
								  $sum_low=$sum_low+1;
								  $total_sum_low=$total_sum_low+$sum_low;
							  }else if($total_con >= 18.50 && $total_con <= 22.99){
								  $action_show="ปกติ";
								    $sum_nor=$sum_nor+1;
									$total_sum_nor=$total_sum_nor+$sum_nor;
							  }else if($total_con > 22.99){
								  $action_show="น้ำหนักเกิน เป็นต้นไป";
								    $sum_over=$sum_over+1;
      ...
      <tr bgcolor="<?php echo $bg;?>" onMouseOver="this.className='menu-over'" onMouseOut="this.className='menu'" class="menu">
        ...
        <td align="center" valign="middle">[<a href="?option=health&task=report_all_type_room&class_code=<?php echo $class_code;?>&room=<?php echo $room;?>&year_in=<?php echo $year_bs;?>&ck=<?php echo $ck;?>&tm=<?php echo $tm;?>">รายละเอียด</a>]</td>
		</tr>
             <?php
			 /*******************************/
	...
	{	
      ...
      <tr bgcolor="<?php echo $bg;?>"  onmouseover="this.className='menu-over'" onMouseOut="this.className='menu'" class="menu">
        ...
        <td align="center" valign="middle"><?php echo $total_nocheck=$check_room_sd2-$checkslw2; echo"$total_nocheck";?></td>
        <td align="center" valign="middle"><?php echo $sum_low;?></td>
        <td align="center" valign="middle"><?php echo $sum_nor;?></td>
        <td align="center" valign="middle"><?php echo $sum_over;?></td>
        <td align="center" valign="middle"><?php echo $sum_tooth_no;?></td>
        ...
        <td align="center" valign="middle"><?php echo number_format($sumtotal,2,'.',',');?> %</td>
        <td align="center" valign="middle"><?php echo number_format($sumtotal_no,2,'.',',');?> %</td>
        <td align="center" valign="middle">[<a href="?option=health&task=report_all_type_room&class_code=<?php echo $class_code;?>&year_in=<?php echo $year_bs;?>&ck=<?php echo $ck;?>&tm=<?php echo $tm;?>">รายละเอียด</a>]</td>
		</tr>
             <?php
			 			 /*******************************/
      ...
      <tr height="30">
        ...
        <td  align="center" valign="middle"><b><?php if($sum_percen_goodT <= 0 || $tosT <= 0){ echo"0.00"; }else{ $verygood = ($sum_percen_goodT*100)/$tosT;?>
		<?php echo number_format($verygood,2,'.',',');   }?> %</b></td>
        <td  align="center" valign="middle"><b><?php  if($sum_percen_noT <= 0 || $tosT <= 0){ echo"0.00"; }else{ $notverygood = ($sum_percen_noT*100)/$tosT;?>
		<?php echo number_format($notverygood,2,'.',',');  }?> %</b></td>
