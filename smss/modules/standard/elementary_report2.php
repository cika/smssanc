...
$sql = "select indicator_name from standard_elementary_indicator where  sd_id='$_REQUEST[sd_id]'  and id_indicator='$_REQUEST[id_indicator]' and sd_year='$_REQUEST[sd_year]'";
$dbquery = mysql_query($sql);
...
echo "<Tr><Td colspan='3' align='right'><INPUT TYPE='button' name='smb' value='ย้อนกลับ' onclick='location.href=\"?option=standard&task=elementary_report2&sd_year_index2=$_REQUEST[sd_year]&budget_year_index2=$_REQUEST[budget_year]\"'</Td></Tr>";
echo "<Tr bgcolor=#FFCCCC><Td width='70' align='center'>ที่</Td><Td align='center'>ชื่อกิจกรรม</Td><Td align='center'>ชื่อโครงการ</Td></Tr>";

		$sql = "select plan_acti.code_acti, plan_acti.code_proj, plan_acti.name_acti from plan_acti, plan_standard where plan_acti.code_acti=plan_standard.code_acti and plan_standard.sd_level='2'  and plan_standard.sd_id='$_GET[sd_id]' and plan_standard.id_indicator='$_GET[id_indicator]' and plan_standard.sd_year='$_GET[sd_year]' and plan_standard.budget_year='$_GET[budget_year]'";
		$N=1;
		$dbquery = mysql_query($sql);
		While ($result = mysql_fetch_array($dbquery)){
...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ตรวจสอบ การมีกิจกรรมสนับสนุนมาตรฐานการศึกษาปฐมวัย</strong></font></td></tr>";
echo "</table>";
...
//เลือกปีงบประมาณ		
		...
		$sql_budgetyear = "select  * from plan_year order by budget_year desc";
...
While ($result = mysql_fetch_array($dbquery)){
	...
	$sql_indicator = "select * from standard_elementary_indicator where sd_id= '$sd_id' and sd_year='$sd_year' order by id_indicator";
		...
		$indicator_name = $result_indicator['indicator_name'];
				...
				// ส่วนหาจำนวนกิจกรรม
				if(($_POST) or (isset($sd_year_index2))){
				$sql_acti= "select count(id) as acti_num from plan_standard where sd_level='2' and  sd_id= '$sd_id'  and  id_indicator= '$id_indicator' and sd_year='$_REQUEST[sdyear_index]' and budget_year='$_REQUEST[budget_year_index]'";	
				}
				else{
				$sql_acti= "select count(id) as acti_num from plan_standard where sd_level='2'  and  sd_id= '$sd_id' and id_indicator= '$id_indicator' and sd_year='$result_sdyear[sd_year]' and budget_year='$result_budget_year[budget_year]'";	
				}
...
echo "<Td><a href='?option=standard&task=elementary_report2&index=1&sd_id=$sd_id&id_indicator=$id_indicator&sd_year=$sd_year&budget_year=$budget_year'><img src=images/browse.png border='0' alt='รายละเอียด'></a></Td>";
}
else{
echo "<Td></Td>";
}
...
function goto_type(val){
	if(val==1){
		callfrm("?option=standard&task=elementary_report2");   // page ย้อนกลับ 
		}
}
</script>
