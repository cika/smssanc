...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>มาตรฐานการศึกษาขั้นพื้นฐาน</strong></font></td></tr>";
echo "</table>";
...
While ($result = mysql_fetch_array($dbquery))
	{
		...
		if($_POST){
		$sql_indicator = "select * from standard_basic_indicator where sd_id= '$sd_id' and sd_year='$_REQUEST[sdyear_index]' order by id_indicator";
		}
		else{
		$sql_indicator = "select * from standard_basic_indicator where sd_id= '$sd_id' and sd_year='$result_sdyear[sd_year]' order by id_indicator";
		}		
...
function goto_type(val){
	if(val==1){
		callfrm("?option=standard&task=basic_report");   // page ย้อนกลับ 
		}
}
</script>
