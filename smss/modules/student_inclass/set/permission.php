...
echo "</table>";
	...
	if($person_id==""){
		...
		if(mysql_num_rows(mysql_db_query($dbname,"Select * from student_inclass_permission where class_now=$class_now and room_now=$room_now and person_id='$person_id' and student_check_year='$year_active'"))>0){
		echo"<br><center>
		<font color=red size=3><b>เกิดข้อผิดพลาด !! คุณเลือกบุคคลที่มีอยู่แล้ว</b><br>
			<a href='$refer'>คลิกที่นี่</a> เพื่อกลับไปทำรายการใหม่
		</center";
		}else{
		$sql="INsert INto student_inclass_permission VAlues($class_now,$room_now,'$person_id','$year_active')";//echo $sql;
		$query=mysql_db_query($dbname,$sql);
?>
<script>
...
$sql = "delete from student_inclass_permission where class_now=$class_now and room_now=$room_now and person_id='$person_id'";
...
echo "<Tr bgcolor='#FFCCCC'><Td  align='center' style='font-weight:bold'>ที่</Td><Td  align='center' style='font-weight:bold'>รหัสประจำตัว</Td><Td  align='center' style='font-weight:bold'>ผู้รับผิดชอบ</Td><Td  align='center' style='font-weight:bold'>การทำรายการ</Td></Tr>";
			...
			$sql_pms="SELECT *  FROM `student_inclass_permission` WHERE `class_now` = $class_now AND `room_now` = $room_now  and student_check_year='$year_active' ";
			$pmsquery = mysql_db_query($dbname, $sql_pms); 
			if(mysql_num_rows($pmsquery)>0){
				$p=1;
				while($pms=mysql_fetch_assoc($pmsquery)){
					$x=$pms['person_id'];
					$color=(($p%2) == 0)?" class='even'":" class='odd'";
					$drop_pic="<a href='?option=student_inclass&task=set/permission&index=3&edu_type=$edu_type&class_now=$class_now&room_now=$room_now&person_id=$x'><img src='images/drop.png' alt='ลบผู้รับผิดชอบจากห้องเรียนนี้' title='ลบผู้รับผิดชอบจากห้องเรียนนี้' border=0></a>";

							$pms_list=$pms_list."<Tr $color><Td align='right' width='35'>$p</Td>
		<Td  align='center'  width=200>$x</Td>
		<Td  align='left'>&nbsp;&nbsp;".getFullnameUser($x)."</Td>
			...
			$report_pic
				...
				$pms_list="<tr class=odd><td colspan=4 align=center><FONT SIZE=2 COLOR=red><B>ยังไม่มีผู้รับผิดชอบ !!</B></FONT></td>";
			}
echo $pms_list;
echo "<tr  bgcolor='#FFCCCC'><td colspan=2 align=left valign=middle ><a href='?option=student_inclass&task=set/permission' alt='กลับไปที่หน้ารายการ' title='กลับไปที่หน้ารายการ' ><img src='images/arrow_left.gif' border=0> <FONT COLOR='#000'>กลับไปที่หน้ารายการ</FONT></a></td>";
...
<div id="dialog" title="เลือกผู้รับผิดชอบที่ต้องการ">
	<p>
	<form name=add_pms action="?option=student_inclass&task=set/permission&index=1" method=post>
...
<INPUT TYPE="hidden" name="edu_type" value="<?php echo $edu_type;?>">
	...
	<option value=''>เลือกผู้ใช้จากรายการ</option>
		<?php
				...
				$query_PM=mysql_db_query($dbname,$sqlPersonMain);
...
$url_link="option=student_inclass&task=set/permission";  // 2_กำหนดลิงค์
...
echo "<Tr><Td colspan='5' align='left'><!--INPUT TYPE='button' name='smb' value='เพิ่มปีการศึกษา' onclick='location.href=\"?option=student_inclass&task=set/year&index=1\"'--></Td></Tr>";
...
While ($result = mysql_fetch_array($dbquery))
	{
		...
		//$year_active = $result['year_active'];
			...
			if(mysql_num_rows($pmsquery)>0){
				...
				while($pms=mysql_fetch_assoc($pmsquery)){
					$x=$pms['person_id'];
							$pms_list=$pms_list."&nbsp;&nbsp;$p. ".getFullnameUser($x)."&nbsp;&nbsp;&nbsp;<br>\n";
								$p++;
				}
			}else{
				$pms_list=" ";
