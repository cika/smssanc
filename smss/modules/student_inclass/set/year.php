<script><?php echo "location.href=\"?option=student_inclass&task=set/period"; ?>";</script>
<?php
...
<Table width='300' Border='0' Bgcolor='#Fcf9d8'>
	...
	$rec_date = date("Y-m-d");
if($_POST[updates]){
	$sqlCHK = mysql_num_rows(mysql_db_query($dbname,"select student_check_year from student_check_year where student_check_year='$_POST[set_year]'"));
		//echo "ggg>".$sqlCHK;
		if($sqlCHK>0 && $_POST[set_year]==$_POST[set_year_old]){
			if($_POST[year_active]=='1'){
				$uquery = mysql_db_query($dbname,"update student_check_year set  year_active='0'");
			}
...
<Table width='300' Border='0' Bgcolor='#Fcf9d8'>
	...
	$sql = "update student_check_year set  year_active='$year_active' where id='$_GET[id]'";
	$dbquery = mysql_db_query($dbname, $sql);
	}
...
$edit_pic='<a href="?option=student_check&task=set/year&index=5&edit_year='.$student_check_year.'"><img src="images/edit.png" alt="แก้ไข" border="0">';
	...
	<?php echo "location.href=\"?option=student_check&task=set/year&id=$_GET[id]&page=$_REQUEST[page]"; ?>";
		}
	}
			function chkForm()
				{
