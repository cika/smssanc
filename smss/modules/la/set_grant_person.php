...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>กำหนดผู้ให้ความเห็นชอบ และผู้อนุมัติ การลา</strong></font></td></tr>";
echo "</table>";
}
echo "<br>";
...
While ($result = mysql_fetch_array($dbquery_1))
	...
	$grant_person="";
		...
		$sql_2 = "select * from la_person_set where person_id='$person_id'";
		$dbquery_2 = mysql_query($sql_2);
		$num=mysql_num_rows($dbquery_2);
			if($num<1 and $rec_index>0){
			$sql_3 = "insert into la_person_set(person_id,comment_person,grant_person,officer,rec_date) values ('$person_id','$comment_person','$grant_person','$officer','$rec_date')";
			$dbquery_3 = mysql_query($sql_3);
			}
			else{
					if($comment_index==1){
					$sql_3 = "update la_person_set set comment_person='$comment_person', officer='$officer', rec_date='$rec_date' where person_id='$person_id'";
					$dbquery_3 = mysql_query($sql_3);
					}
					if($grant_index==1){
					$sql_4 = "update la_person_set set grant_person='$grant_person', officer='$officer', rec_date='$rec_date' where person_id='$person_id'";
...
$sql = "select * from  la_person_set where id='$_GET[id]'";
...
$sql_num = "select  * from la_person_set where id='$_POST[id]'";
$dbquery_num = mysql_query($sql_num);
$num_rows=mysql_num_rows($dbquery_num);
		if($num_rows>0){
		$sql = "update la_person_set set comment_person='$_POST[comment_person]', grant_person='$_POST[grant_person]', officer='$officer', rec_date='$rec_date' where id='$_POST[id]'";
		$dbquery = mysql_query($sql);
		}
		else{
		$sql_3 = "insert into la_person_set(person_id,comment_person,grant_person,officer,rec_date) values ('$_POST[person_id]','$_POST[comment_person]','$_POST[grant_person]','$officer','$rec_date')";
		$dbquery_3 = mysql_query($sql_3);
		}	
}
...
echo "<Tr align='center'><Td colspan='3'></Td><Td><font color='#006666' size='2'>ผู้ให้ความเห็นชอบ</font></Td><Td><font color='#006666' size='2'>ผู้อนุมัติ</font></Td><td></td></Tr>";

echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='30'>ที่</Td><Td width='250'>ชื่อ</Td><Td>ตำแหน่ง</Td><Td width='150'>";
...
$sql = "select la_person_set.id, person_main.person_id, person_main.prename, person_main.name, person_main.surname, person_main.position_code, la_person_set.comment_person, la_person_set.grant_person from person_main left join la_person_set on person_main.person_id=la_person_set.person_id where person_main.status='0' order by person_main.position_code,person_main.person_order";
$dbquery = mysql_query($sql);
...
echo "</td>";
echo "<Td valign='top' align='center'><a href=?option=la&task=set_grant_person&index=5&id=$id&person_id=$person_id><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
echo "</Tr>";
$M++;
$N++;
	}
...
function goto_url(val){
	if(val==0){
		callfrm("?option=la&task=set_grant_person");   // page ย้อนกลับ 
	}else if(val==1){
			callfrm("?option=la&task=set_grant_person&index=4");   //page ประมวลผล
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.grant_person.value == ""){
			...
			callfrm("?option=la&task=set_grant_person&index=6");   //page ประมวลผล
		}
	}
}
</script>
