...
echo "<tr align='center'><td><font color='#006666' size='2'><strong>ทะเบียนยกเลิกการลา</strong></font></td></tr>";
echo "</table>";
}
...
$date_time_now = date("Y-m-d H:i:s");
		$sql = "update la_cancel set group_comment='$_POST[group_comment]', 
		group_sign='$_SESSION[login_user_id]', 
...
//ส่วนของการแยกหน้า
$sql = "select la_cancel.id from la_cancel left join la_person_set on la_cancel.person_id=la_person_set.person_id where la_person_set.comment_person ='$_SESSION[login_user_id]' ";
$dbquery = mysql_query($sql);
$num_rows = mysql_num_rows($dbquery );

$pagelen=20;  // 1_กำหนดแถวต่อหน้า
$url_link="option=la&task=main/cancel_basic_comment";  // 2_กำหนดลิงค์ฺ
...
$sql = "select la_cancel.id, la_cancel.person_id, la_cancel.rec_date, la_cancel.la_type, la_cancel.cancel_la_start, la_cancel.cancel_la_finish, la_cancel.cancel_la_total, la_cancel.group_sign, la_cancel.commander_sign from la_cancel left join la_person_set on la_cancel.person_id=la_person_set.person_id where la_person_set.comment_person ='$_SESSION[login_user_id]' order by la_cancel.id limit $start,$pagelen";

$dbquery = mysql_query($sql);

echo  "<table width='98%' border='0' align='center'>";

echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='60'>เลขที่</Td><Td width='120'>ผู้ขออนุญาต</Td><Td width='100'>วันขออนุญาต</Td><Td>ประเภทการลา</Td><Td>ตั้งแต่วันที่</Td><Td>ถึงวันที่</Td><Td>มีกำหนด</Td><Td width='50'>รายละเอียด</Td><Td>สถานะ<br>การลงนาม</Td><Td width='40'>ลงนาม</Td></Tr>";
...
echo "<Td align='center'><a href=?option=la&task=main/cancel_basic_comment&index=7&id=$id&page=$page><img src=images/browse.png border='0' alt='รายละเอียด'></Td>";
...
echo "<Td valign='top' align='center'><a href=?option=la&task=main/cancel_basic_comment&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='บันทึก'></a></Td>";
}
else{
echo "<Td></td>";
}
...
function goto_url_update(val){
	if(val==0){
		callfrm("?option=la&task=main/cancel_basic_comment");   // page ย้อนกลับ 
	}else if(val==1){
			callfrm("?option=la&task=main/cancel_basic_comment&index=6");   //page ประมวลผล
	}
}
