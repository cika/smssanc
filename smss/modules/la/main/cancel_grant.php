...
if ($index==6){
$date_time_now = date("Y-m-d H:i:s");
		$sql = "update la_cancel set commander_comment='$_POST[commander_comment]', 
...
$sql = "select la_cancel.id, la_cancel.person_id, la_cancel.rec_date, la_cancel.la_type, la_cancel.cancel_la_start, la_cancel.cancel_la_finish, la_cancel.cancel_la_total, la_cancel.grant_p_selected, la_cancel.group_sign,la_cancel.commander_grant,la_cancel.commander_comment,la_cancel.commander_sign,la_cancel.grant_date from la_cancel left join la_person_set on  la_cancel.person_id=la_person_set.person_id where (la_cancel.grant_p_selected ='$_SESSION[login_user_id]') or (la_person_set.grant_person ='$_SESSION[login_user_id]') and (la_cancel.rec_date<'$onedayago' or la_person_set.comment_person is null or la_person_set.comment_person='' or la_cancel.no_comment='1' or la_cancel.group_sign is not null) ";

$dbquery = mysql_query($sql);
$num_rows = mysql_num_rows($dbquery );

$pagelen=20;  // 1_กำหนดแถวต่อหน้า
$url_link="option=la&task=main/cancel_grant";  // 2_กำหนดลิงค์ฺ
...
$sql = "select la_cancel.id, la_cancel.person_id, la_cancel.rec_date, la_cancel.la_type, la_cancel.cancel_la_start, la_cancel.cancel_la_finish, la_cancel.cancel_la_total, la_cancel.grant_p_selected, la_cancel.group_sign,la_cancel.commander_grant,la_cancel.commander_comment,la_cancel.commander_sign,la_cancel.grant_date from la_cancel left join la_person_set on  la_cancel.person_id=la_person_set.person_id where (la_cancel.grant_p_selected ='$_SESSION[login_user_id]') or (la_person_set.grant_person ='$_SESSION[login_user_id]') and (la_cancel.rec_date<'$onedayago' or la_person_set.comment_person is null or la_person_set.comment_person='' or la_cancel.no_comment='1' or la_cancel.group_sign is not null) order by la_cancel.id limit $start,$pagelen";

$dbquery = mysql_query($sql);

echo  "<table width='98%' border='0' align='center'>";

echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='60'>เลขที่</Td><Td width='120'>ผู้ขออนุญาต</Td><Td width='100'>วันขออนุญาต</Td><Td>ประเภทการลา</Td><Td>ตั้งแต่วันที่</Td><Td>ถึงวันที่</Td><Td>มีกำหนด</Td><Td width='50'>รายละเอียด</Td><Td>คำสั่ง/อนุมัติ</Td><Td width='40'>บันทึก</Td></Tr>";
...
continue;
		...
		$la_start = $result['cancel_la_start'];
		$la_finish = $result['cancel_la_finish'];
		$la_total = $result['cancel_la_total'];
...
echo "<Td align='center'><a href=?option=la&task=main/cancel_grant&index=7&id=$id&page=$page><img src=images/browse.png border='0' alt='รายละเอียด'></Td>";

if($result['commander_sign']==""){
...
if($now<$timestamp_2){
echo "<Td valign='top' align='center'><a href=?option=la&task=main/cancel_grant&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='บันทึก'></a></Td>";
}
else{
echo "<Td></td>";
}
...
function goto_url_update(val){
	if(val==0){
		callfrm("?option=la&task=main/cancel_grant");   // page ย้อนกลับ 
	}else if(val==1){
		if(!(frm1.commander_grant[0].checked || frm1.commander_grant[1].checked)){
			alert("กรุณาเลือกการอนุมัติ");
		}else{
			callfrm("?option=la&task=main/cancel_grant&index=6");   //page ประมวลผล
		}	
	}
}
</script>
