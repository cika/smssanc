...
echo "<Tr align='left'><Td align='right'>ไม่ต้องผ่านผู้บังคับบัญชาขั้นต้น&nbsp;&nbsp;</Td><Td><input type='checkbox'  name='no_comment' id='no_comment' value='1' $no_comment_select>&nbsp;&nbsp;(เลือกกรณีผู้บังคับบัญชาขั้นต้นไม่ได้ปฏิบัติราชการ)</Td></Tr>";
		...
		echo "</select>";
		echo "&nbsp;&nbsp;(ใช้กรณีผู้อนุมัติิปกติไม่อย่) </Td></Tr>";
...
$date_time_now = date("Y-m-d H:i:s");
		$sql = "update la_main set group_comment='$_POST[group_comment]', 
...
echo "<Tr ><Td colspan='2' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=la&task=main/basic_comment&page=$_GET[page]\"'></Td></Tr>";
...
//ส่วนของการแยกหน้า
$sql = "select la_main.id from la_main left join la_person_set on la_main.person_id=la_person_set.person_id where la_person_set.comment_person ='$_SESSION[login_user_id]' ";
$dbquery = mysql_query($sql);
$num_rows = mysql_num_rows($dbquery );

$pagelen=20;  // 1_กำหนดแถวต่อหน้า
$url_link="option=la&task=main/basic_comment";  // 2_กำหนดลิงค์ฺ
...
$sql = "select la_main.id, la_main.person_id, la_main.rec_date, la_main.la_type, la_main.la_start, la_main.la_finish, la_main.la_total, la_main.document, la_main.group_sign, la_main.commander_sign from la_main left join la_person_set on la_main.person_id=la_person_set.person_id where la_person_set.comment_person ='$_SESSION[login_user_id]' order by la_main.id limit $start,$pagelen";

$dbquery = mysql_query($sql);

echo  "<table width='98%' border='0' align='center'>";

echo "<Tr bgcolor='#FFCCCC' align='center'><Td width='60'>เลขที่</Td><Td width='120'>ผู้ขออนุญาต</Td><Td width='100'>วันขออนุญาต</Td><Td>ประเภทการลา</Td><Td>ตั้งแต่วันที่</Td><Td>ถึงวันที่</Td><Td>มีกำหนด</Td><Td>เอกสาร</Td><Td width='50'>รายละเอียด</Td><Td>สถานะ<br>การลงนาม</Td><Td width='40'>ลงนาม</Td></Tr>";
...
echo "<Td align='center'><a href=?option=la&task=main/basic_comment&index=7&id=$id&page=$page><img src=images/browse.png border='0' alt='รายละเอียด'></Td>";
...
echo "<Td valign='top' align='center'><a href=?option=la&task=main/basic_comment&index=5&id=$id&page=$page><img src=images/edit.png border='0' alt='บันทึก'></a></Td>";
}
else{
echo "<Td></td>";
}
...
function goto_url_update(val){
	if(val==0){
		callfrm("?option=la&task=main/basic_comment&page=<?=$_REQUEST[page]?>");   // page ย้อนกลับ 
	}else if(val==1){
			callfrm("?option=la&task=main/basic_comment&index=6");   //page ประมวลผล
	}
}
