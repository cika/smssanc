...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=work&task=permission&index=3&id=$_GET[id]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=work&task=permission\"'";
echo "</td></tr></table>";
}

//ส่วนลบข้อมูล
if($index==3){
$sql = "delete from work_permission where id=$_GET[id]";
$dbquery = mysql_query($sql);
...
if($index==4){
$rec_date = date("Y-m-d");
$sql = "insert into work_permission (person_id, p1, officer,rec_date) values ('$_POST[person_id]', '$_POST[work_permission1]','$_SESSION[login_user_id]','$rec_date')";
$dbquery = mysql_query($sql);
echo "<script>document.location.href='?option=work&task=permission'; </script>\n";
}
...
echo "<Br><Br>";
echo "<Table width='50%' Border= '0' Bgcolor='#Fcf9d8'>";
$sql = "select * from work_permission where id='$_GET[id]'";
$dbquery = mysql_query($sql);
...
if ($index==6){
$rec_date = date("Y-m-d");
$sql = "update work_permission set  person_id='$_POST[person_id]', p1='$_POST[work_permission1]', officer='$_SESSION[login_user_id]', rec_date='$rec_date' where id='$_POST[id]'";
$dbquery = mysql_query($sql);
echo "<script>document.location.href='?option=work&task=permission'; </script>\n";
}
...
$dbquery = mysql_query($sql);
echo  "<table width=50% border=0 align=center>";
echo "<Tr><Td colspan='5' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มเจ้าหน้าที่' onclick='location.href=\"?option=work&task=permission&index=1\"'</Td></Tr>";
...
function goto_url(val){
	if(val==0){
		...
		if(frm1.person_id.value == ""){
			...
			callfrm("?option=work&task=permission&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.person_id.value == ""){
			...
			callfrm("?option=work&task=permission&index=6");   //page ประมวลผล
		}
	}
}
</script>
