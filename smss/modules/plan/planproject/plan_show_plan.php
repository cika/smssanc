...
?>
<p align="center" STYLE="font-family: 'sans-serif', fantasy; font-size: 13pt; color:blue;font-weight: bold;">โครงการ จำแนกตามกลุ่ม(งาน)</Font></p>
<!-- Part2	## -->
<FORM  name="frm1" >
<?php
//เลือกปี และกลุ่ม
					...
					While ($result_acti = mysql_fetch_array($dbquery_acti))
						{
							...
							$code_approve= $result_acti['code_approve'];
...
echo   "<Td  width='12%' valign='center'  align='center' ><font  size='2'>แหล่งงบประมาณ</font></td>";
...
{		
		...
		echo "<Tr bgcolor='#CCFFFF'><Td align=center><font  size='2'>$i</font></Td><Td align=center><font  size='2'>$code_proj_plan_ar[$i]</font></Td><Td><font  size='2'>$name_proj_plan_ar[$i]</font></Td><Td align=right><font  size='2'>$budget_proj</font></Td><td></td><Td align=left><font  size='2'>";
echo $person_ar[$owner_proj_plan_ar[$i]];
echo "</font></Td>";	
	
		if($file_detail_plan_ar[$i] !=""){
		echo "<Td align='center'> <a href='$file_detail_plan_ar[$i]' target='_blank' title='อ่านโครงการ'><img src='images/b_browse.png' width='16' height='16' border='0'></a></td>";	
		 }
		 else{
		 echo "<Td>&nbsp;</td>";
		}
...
echo "</tr>";
		...
		for($x=1;$x<$re;$x++)
		{
				...
				if($code_approve_ar[$x]>0 and $code_approve_ar[$x]<200){
				$sql_type ="select *	from budget_type where type_id='$code_approve_ar[$x]' ";			
				$dbquery_type =DBfieldQuery($sql_type);
				$result_type = mysql_fetch_array($dbquery_type);
				echo "<Td>$result_type[type_name]</td>";
				}
				else if($code_approve_ar[$x]>=400){
				$bud_type_id=$code_approve_ar[$x]-400;
				$sql_type ="select item from  budget_bud where status='1' and budget_year='$proj_year' and id='$bud_type_id' ";		
				$dbquery_type =DBfieldQuery($sql_type);
				$result_type = mysql_fetch_array($dbquery_type);
				echo "<Td>$result_type[item]</td>";
				}
				else{
				echo "<Td><font color='#FF0000'>ยังไม่ได้กำหนด</font></td>";
				}
...
echo "<Td colspan='3' align='center'>รวม</Td><td>$budget_proj_sum</td><Td colspan='3'>&nbsp;</Td></Tr></Table>";
...
callfrm("?option=plan&task=planproject/plan_show_plan"); 		
}
