...
?>
<p align="center" STYLE="font-family: 'sans-serif', fantasy; font-size: 13pt; color:blue;font-weight: bold;">โครงการ จำแนกตามกลยุทธ์</Font></p>
<!-- Part2	## -->
<FORM  name="frm1" >
<?php
//เลือกประเภท
...
//////////////////	
	...
	echo "</select>&nbsp;";
/////////////////////
	echo "<Select  name='plan_tegic' size='1'>";
	echo  '<option value ="" >ทุกกลยุทธ์</option>' ;
	$sql = "SELECT *  FROM  plan_stregic where budget_year='$proj_year' ";
	$dbquery =DBfieldQuery($sql);
			While ($result = mysql_fetch_array($dbquery))
			{ 
					if ($plan_tegic==$result['id_tegic']){
					echo "<option value=$result[id_tegic]  selected>$result[strategic]</option>"; 
					} 
					else{
					echo "<option value=$result[id_tegic]>$result[strategic]</option>"; 
					}
			}
					echo "</select>";
...
ShowProj($plan_tegic,$proj_year);
#######################################
function ShowProj($plan_tegic,$proj_year){
//อาเรย์บุคลากร
...
$person_ar[$result['person_id']]=$result['prename'].$result['name']." ".$result['surname'];
 ...
 if($plan_tegic!="")
$sql = "select * from  plan_proj  where (code_tegy='$plan_tegic')  and  (budget_year='$proj_year') order by code_proj ";
else
...
echo   "<Td  width='20%'  valign='center'  align='center' ><font  size='2'>หัวหน้าโครงการ</font></td>";
...
callfrm("?option=plan&task=planproject/plan_show_plan2"); 		
}
