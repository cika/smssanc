...
//$sql = "SELECT *  FROM  standard_basic_sd  where sd_year=$fetyear[sd_year]  ORDER BY sd_id  ASC ";
$sql = "SELECT *  FROM  standard_basic_sd  where sd_year=$fetyear  ORDER BY sd_id  ASC ";
...
//===============
$sql = "SELECT *  FROM  standard_basic_indicator where (sd_id=$fieldb[$rd] and sd_year=$fetyear) ORDER BY id_indicator  ASC ";
...
While($fetcharr = mysql_fetch_array($result2) ) { 
	...
	$sql = "SELECT *  FROM  plan_standard  where (code_acti='$code_acti'  and budget_year='$budget_year' and sd_year='$fetyear'  and sd_level='$sd_level'  and sd_id='$fetcharr[sd_id]'  and  id_indicator='$fetcharr[id_indicator]')  limit 1";
