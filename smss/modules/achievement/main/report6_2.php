...
$sql = "select distinct ed_year from achievement_main where  test_type='3' and test_class='2' order by ed_year desc ";
...
for($x=1;$x<4;$x++){
			if($ed_year==""){
			$strQuery = "select thai, math, science, social, english, health, art, vocation, score_avg from achievement_main where test_type='3' and test_class='2' and ed_year='$year_ar[1]' and level='$x' ";
			}
			else{
...
function goto_display(val){
	if(val==1){
		callfrm("?option=achievement&task=main/report6_2"); 
		}
}
</script>
