...
<CENTER>
<h2>คะแนนสอบ LAST</h2>
<h3>ชั้นประถมศึกษาปีที่ 5</h3>
<?php
echo "<form id='frm1' name='frm1'>";
echo "<table width='95%' align='center'><tr><td align='right'>";
echo "<Select  name='ed_year' size='1'>";
$sql = "select distinct ed_year from achievement_main  where  achievement_main.test_type='3' and achievement_main.test_class='5' order by ed_year desc limit 4";
...
$strXML = "<graph xaxisname='กลุ่มสาระ' yaxisname='Score' hovercapbg='DEDEBE' hovercapborder='889E6D' rotateNames='0' yAxisMaxValue='100' numdivlines='9' divLineColor='CCCCCC' divLineAlpha='80' decimalPrecision='0' showAlternateHGridColor='1' AlternateHGridAlpha='30' AlternateHGridColor='CCCCCC' caption='' subcaption='' >";
  ...
  for($x=3;$x>0;$x--){
  			...
  			$strQuery = "select  thai, math, science, social,  english, health, art, vocation, score_avg from achievement_main where  test_type='3' and test_class='5' and  ed_year='$ed_year_ar[1]'  and level='$x' ";
			}
  			else {
  			$strQuery = "select  thai, math, science, social,  english, health, art, vocation, score_avg from achievement_main where  test_type='3' and test_class='5' and  ed_year='$ed_year'  and  level='$x' ";
			}
...
function goto_display(val){
	if(val==1){
		callfrm("?option=achievement&task=main/report71"); 
		}
}
</script>