...
$strXML = "<graph xaxisname='กลุ่มสาระ' yaxisname='Score' hovercapbg='DEDEBE' hovercapborder='889E6D' rotateNames='0' yAxisMaxValue='100' numdivlines='9' divLineColor='CCCCCC' divLineAlpha='80' decimalPrecision='0' showAlternateHGridColor='1' AlternateHGridAlpha='30' AlternateHGridColor='CCCCCC' caption='' subcaption='' >";
  ...
  //ส่วนของปีการศึกษา 
 	$strQuery = "select distinct ed_year from achievement_main where  test_type='1' and test_class='9' order by ed_year desc limit 4";
...
$year_num=$year_num-1;  //จำนวนปีที่แสดง
  ...
  for($x=$year_num;$x>0;$x--){
	$strQuery = "select thai, math, science, social, english, health, art, vocation, score_avg from achievement_main where test_type='1' and test_class='9' and ed_year='$ed_year_ar[$x]' and level='1' ";
			$result = mysql_query($strQuery);
...
function goto_display(val){
	if(val==1){
		callfrm("?option=achievement&task=main/report2"); 
		}
}
</script>
