...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>นักเรียนย้ายโรงเรียน</strong></font></td></tr>";
echo "</table>";
}
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>แก้ไขข้อมูลการย้ายโรงเรียน</B></Font>";
echo "</Cener>";
echo "<Br><Br>";
...
echo "<Tr align='left'><Td align='right'>การย้ายโรงเรียน&nbsp;&nbsp;</Td><Td>";
...
echo  "<option  value = '0'>ไม่ย้ายโรงเรียน</option>" ;
echo  "<option  value = '2' $select>ย้ายโรงเรียน</option>" ;
echo "</select>";
echo "</Td></Tr>";
...
if($index==9){
echo "<table width='50%' border='0' align='center'>";
echo "<tr align='center'><td><font color='#006666' size='3'><strong>เพิ่มนักเรียนย้ายโรงเรียน</strong></font></td></tr>";
echo "</table>";
	//ส่วนของการแยกหน้า
$pagelen=45;  // 1_กำหนดแถวต่อหน้า
$url_link="option=student_main&task=tranfer&index=9";  // 2_กำหนดลิงค์ฺ
if($class_index!=""){
		if($room_index>=1){
...
//เลือกปีการศึกษา		
		...
		While ($result = mysql_fetch_array($dbquery))
		   {
				if($year_index!=""){
				$year_active_result['ed_year']=$year_index;
				}
...
echo "<Tr bgcolor='#FFCCCC'><Td colspan='11'><input type='checkbox' name='allchk' id='allchk' onclick='CheckAll()'>เลือก/ไม่เลือกทั้งหมด &nbsp;&nbsp;&nbsp;&nbsp;<INPUT TYPE='button' name='smb' value='นักเรียน(ที่เลือก)ย้ายโรงเรียน' onclick='goto_finish_all()'></Td></Tr>";
...
if($index==10){
	...
	$sql = "update student_main set status='2', out_edyear='$year_index' where student_id='$student_id'";
	$dbquery = mysql_query($sql);
	}
}
...
//ส่วนของการแยกหน้า
$pagelen=45;  // 1_กำหนดแถวต่อหน้า
$url_link="option=student_main&task=tranfer";  // 2_กำหนดลิงค์ฺ
if($class_index!=""){
		if($room_index>=1){
		$sql = "select * from student_main where out_edyear='$year_index' and class_now='$class_index' and room='$room_index' and status='2'  ";
		}
		else{
		$sql = "select * from student_main where out_edyear='$year_index' and class_now='$class_index' and status='2' ";
		}
}
else{
$sql = "select * from student_main where out_edyear='$year_index' and status='2'";
}
$dbquery = mysql_query($sql);
$num_rows=mysql_num_rows($dbquery);
$totalpages=ceil($num_rows/$pagelen);
...
//จบแยกหน้า
		...
		echo "<INPUT TYPE='button' name='smb' value='เพิ่มข้อมูลนักเรียนย้ายโรงเรียน' onclick='location.href=\"?option=student_main&task=tranfer&index=9\"'>";
		echo "</td>";
...
//เลือกห้อง		
if($class_index!=""){
		$sql_room = "select distinct room from student_main where  out_edyear='$year_index' and class_now='$class_index' and room >= '1' and status='2' order by room";
		$dbquery_room = mysql_query($sql_room);
		$room_num= mysql_num_rows($dbquery_room);
		if($room_num>=1){
		echo " <Select  name='room_index' size='1'>";
...
if($class_index!=""){
		if($room_index>=1){
		$sql = "select * from student_main where out_edyear='$year_index' and class_now='$class_index' and room='$room_index' and status='2' order by class_now,room,student_number,student_id limit $start,$pagelen";
		}
		else{
		$sql = "select * from student_main where out_edyear='$year_index' and class_now='$class_index' and status='2' order by class_now,room,student_number,student_id limit $start,$pagelen";
		}
}
else{
$sql = "select * from student_main where out_edyear='$year_index' and status='2' order by class_now,room,student_number,student_id limit $start,$pagelen";
}
$dbquery = mysql_query($sql);
echo  "<table width='95%' border='0' align='center'>";
...
echo "<td><a href=?option=student_main&task=tranfer&index=5&id=$id&page=$page&class_index=$class_index&room_index=$room_index><img src=images/edit.png border='0' alt='แก้ไข'></a></td>";
echo "</Tr>";
...
function goto_url_update(val){
	if(val==0){
		callfrm("?option=student_main&task=tranfer");   // page ย้อนกลับ 
	}else if(val==1){
	callfrm("?option=student_main&task=tranfer&index=6");   //page ประมวลผล
	}
}
...
function goto_index(val){
	if(val==1){
		...
		callfrm("?option=student_main&task=tranfer&index=9");   // page ย้อนกลับ 
		}
}

function goto_finish_all(){
			callfrm("?option=student_main&task=tranfer&index=10"); 
}
