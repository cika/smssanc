...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=permission&task=permission&index=3&id=$_GET[id]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=permission&task=permission\"'";
echo "</td></tr></table>";
}

//ส่วนลบข้อมูล
if($index==3){
$sql = "delete from permission_permission where id=$_GET[id]";
$dbquery = mysql_query($sql);
}
...
if($index==4){
$rec_date = date("Y-m-d");
$sql = "insert into permission_permission (person_id, p1, officer,rec_date) values ('$_POST[person_id]', '$_POST[work_permission1]','$_SESSION[login_user_id]','$rec_date')";
$dbquery = mysql_query($sql);
}
...
echo "<Br><Br>";
echo "<Table width='50%' Border= '0' Bgcolor='#Fcf9d8'>";
$sql = "select * from permission_permission where id='$_GET[id]'";
$dbquery = mysql_query($sql);
...
if ($index==6){
$rec_date = date("Y-m-d");
$sql = "update permission_permission set  person_id='$_POST[person_id]', p1='$_POST[work_permission1]', officer='$_SESSION[login_user_id]', rec_date='$rec_date' where id='$_POST[id]'";
$dbquery = mysql_query($sql);
echo "<script>document.location.href='?option=permission&task=permission'; </script>\n";
}

//ส่วนแสดงผล
if(!(($index==1) or ($index==2) or ($index==5))){

$sql = "select permission_permission.id, permission_permission.p1, person_main.name, person_main.surname from permission_permission left join person_main on permission_permission.person_id=person_main.person_id order by permission_permission.id";
$dbquery = mysql_query($sql);
echo  "<table width=50% border=0 align=center>";
echo "<Tr><Td colspan='5' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มเจ้าหน้าที่' onclick='location.href=\"?option=permission&task=permission&index=1\"'</Td></Tr>";
...
While ($result = mysql_fetch_array($dbquery))
	{
		...
		<Td align='center' width='50' ><a href=?option=permission&task=permission&index=2&id=$id><img src=images/drop.png border='0' alt='ลบ'></a></Td>
		<Td align='center' width='50'><a href=?option=permission&task=permission&index=5&id=$id><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>
	</Tr>";
$M++;
	}
echo "</Table>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=permission&task=permission");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.person_id.value == ""){
			alert("กรุณาเลือกบุคลากร");
		}else{
			callfrm("?option=permission&task=permission&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.person_id.value == ""){
			...
			callfrm("?option=permission&task=permission&index=6");   //page ประมวลผล
		}
	}
}
</script>
