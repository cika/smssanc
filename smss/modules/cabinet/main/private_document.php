...
echo "<INPUT TYPE='button' name='smb' value='ยืนยัน' onclick='location.href=\"?option=cabinet&task=main/private_document&index=3&id=$_REQUEST[id]&page=$_REQUEST[page]&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]\"'>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ยกเลิก' onclick='location.href=\"?option=cabinet&task=main/private_document&index=7&page=$_REQUEST[page]&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]\"'";
echo "</td></tr></table>";
}
...
echo "<script>document.location.href='?option=cabinet&task=main/private_document&index=7&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]'; </script>\n";
}
...
echo "<INPUT TYPE='button' name='smb' value='ตกลง' onclick='goto_url_update(1)' class=entrybutton>
		&nbsp;&nbsp;<INPUT TYPE='button' name='back' value='ย้อนกลับ' onclick='location.href=\"?option=cabinet&task=main/private_document&index=7&page=$_REQUEST[page]&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]\"'";
echo "</form>";
}
...
$url_link="option=cabinet&task=main/private_document&index=7&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]";  // 2_กำหนดลิงค์
...
echo "<Tr >";
			if($_REQUEST['tray_id']==$user){
			echo "<Td colspan='4' align='left'><INPUT TYPE='button' name='smb' value='เพิ่มเอกสารในแฟ้ม$result_file[file_name]' onclick='location.href=\"?option=cabinet&task=main/private_document&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]&add_index=1\"'></Td>";
			}
			else{
			echo "<td colspan='4'></td>";
			}
echo "<Td colspan='5' align='right'><INPUT TYPE='button' name='smb' value='<<กลับไปตู้เอกสาร' onclick='location.href=\"?option=cabinet&task=main/private_document&person_switch=$_REQUEST[tray_id]\"'></Td></Tr>";
...
echo "<Td align='center'><a href=?option=cabinet&task=main/private_document&index=2&id=$result[id]&page=$page&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]><img src=images/drop.png border='0' alt='ลบ'></a></Td><Td align='center'><a href=?option=cabinet&task=main/private_document&index=5&id=$result[id]&page=$page&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]><img src=images/edit.png border='0' alt='แก้ไข'></a></Td>";
}
else {
echo "<td></td><td></td>";
}
...
echo "<Select  name='person_switch' size='1'>";
echo  '<option value ="" >เลือก</option>' ;
...
echo "<Td></Td><Td></Td><Td></Td></Tr>";
			...
			While ($result_tray = mysql_fetch_array($dbquery_tray)){
							...
							if(($_GET['add_index']==1) and ($result['cabinet_id']==$_REQUEST['cabinet_id'])  and  ($result_tray['tray_id']==$_REQUEST['tray_id'])){
									if($size_sum<100){
									echo "<script>document.location.href='?option=cabinet&task=main/private_document&cabinet_id=$_REQUEST[cabinet_id]&tray_id=$_REQUEST[tray_id]&file_id=$_REQUEST[file_id]&index=1';</script>\n";
									}
									else{
									echo "<script>alert('ลิ้นชักเต็ม ติดต่อผู้จัดการตู้เอกสาร');</script>";
									echo "<script>document.location.href='?option=cabinet&task=main/private_document';</script>";
									}
							}
							//จบส่วนแยก
			...
			//จบส่วนลิ้นชัก
							...
							echo "<Tr  bgcolor='$Fcolor' align='center'><Td></Td><td></td><td></td><td colspan='3' align='left'>แฟ้มเลขที่&nbsp;$result_file[file_id]&nbsp;$result_file[file_name]</td><Td>$result_file_num[file_num]</Td><Td><a href=?option=cabinet&task=main/private_document&index=7&cabinet_id=$result[cabinet_id]&tray_id=$result_tray[tray_id]&file_id=$result_file[file_id]><img src=images/b_browse.png border='0' alt='ดู'></Td>";
							if(($result_tray['tray_id']==$user) and ($size_sum<100)){
							echo "<Td><a href=?option=cabinet&task=main/private_document&index=1&cabinet_id=$result[cabinet_id]&tray_id=$result_tray[tray_id]&file_id=$result_file[file_id]><img src=images/edit.png border='0' alt='เพิ่ม'></a></Td>";
							}
							else{
							echo "<td></td>";
							}
...
function goto_url(val){
	if(val==0){
		callfrm("?option=cabinet&task=main/private_document");   // page ย้อนกลับ 
	}else if(val==1){
		if(frm1.doc_subject.value == ""){
			...
			callfrm("?option=cabinet&task=main/private_document&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		callfrm("?option=cabinet&task=main/private_document");   // page ย้อนกลับ 
			...
			callfrm("?option=cabinet&task=main/private_document&index=6");   //page ประมวลผล
		}
	}
}
...
function goto_display(val){
	if(val==1){
		callfrm("?option=cabinet&task=main/private_document"); 
		}
}
</script>
